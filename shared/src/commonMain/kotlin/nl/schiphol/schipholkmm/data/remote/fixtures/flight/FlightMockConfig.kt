...
import nl.schiphol.schipholkmm.data.remote.support.respondJson
    ...
    return when {
        requestData.url.fullPath.endsWith("flight/${FlightRouteFixture.id}") ||
                (requestData.url.fullPath.contains("flight") &&
                        requestData.url.fullPath.endsWith(FlightRouteFixture.number)) -> respondJson(
            FlightRouteFixture.jsonOk()
        )
